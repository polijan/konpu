...
#include "platform_.h"
   ...
   void UtilSleep(int ms) { if (ms > 0) SDL_Delay(ms); }
#elif KONPU_PLATFORM_WINDOWS
   void UtilSleep(int ms) { if (ms > 0) Sleep(ms); }
...
#  else
#     include <unistd.h>
      void UtilSleep(int ms) { if (ms > 0) usleep(ms * 1000); }
#  endif
...
#else
   void    TimeInit(void)  {}
   int64_t UtilTime(void)  { return -1; }
#endif
